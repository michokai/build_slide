...
ctheme=themechk((s.splitlines())[0])
print(f"beamerテーマ: {ctheme}")

print(buld_slide.parent/tagdir)

tempdic={"SimpleDarkBlue":"main_template.tex", "metropolis":"metro_template.tex"}

# 3. main_temp.texの生成　imagesディレクトリをターゲットディレクトリ名にする(footerにディレクトリ表示をする）
with open(buld_slide / 'templates' / tempdic[ctheme], encoding='utf-8') as f_in, \
...
with open(main_tex, "w", encoding="utf-8") as out:
    ...
    out.write(s[fpoint:tpoint]) # None,Noneならば全件出力
...
cmd = [
    "latexmk",
    '-pdflatex=lualatex -shell-escape -interaction=nonstopmode -halt-on-error -file-line-error %O %S',
    "main.tex",
]
